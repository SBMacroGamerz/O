...
class WebRtcClient(
     ...
     * screen and camera are not mixed simultaneously, which avoids
     * renegotiation complexity for what is a secondary/occasional feature
     * here rather than the primary use case.
     */
    fun startCameraCapture(useFrontCamera: Boolean = false): Boolean {
        stopCurrentVideoCapture()
        ...
        val capturer = enumerator.createCapturer(deviceName, null) ?: return false
        videoCapturer = capturer

        val surfaceHelper = SurfaceTextureHelper.create("CameraCaptureThread", eglBase.eglBaseContext)
        videoSource = factory.createVideoSource(false)
        capturer.initialize(surfaceHelper, context, videoSource!!.capturerObserver)
        capturer.startCapture(1280, 720, 30)

        localVideoTrack = factory.createVideoTrack("O_CAMERA_TRACK", videoSource)
        replaceOutgoingVideoTrack()
        return true
    }

    /** Switches the outgoing video source back to the screen. */
    fun switchToScreenCapture(resultCode: Int, resultData: Intent) {
        stopCurrentVideoCapture()
        startScreenCapture(resultCode, resultData)
        replaceOutgoingVideoTrack()
    }

    private fun stopCurrentVideoCapture() {
    ...
    /** Replaces the video track on the existing sender without a full renegotiation where possible. */
    private fun replaceOutgoingVideoTrack() {
        val sender = peerConnection?.senders?.firstOrNull { it.track()?.kind() == "video" }
        if (sender != null) {
            sender.setTrack(localVideoTrack, false)
        } else {
            localVideoTrack?.let { peerConnection?.addTrack(it, listOf("O_STREAM")) }
        }
    }

    /** HOST ONLY: enable microphone audio as an additional outgoing track. */
    fun startAudioCapture() {
        if (localAudioTrack != null) return
        val audioConstraints = MediaConstraints()
        audioSource = factory.createAudioSource(audioConstraints)
        localAudioTrack = factory.createAudioTrack("O_AUDIO_TRACK", audioSource)
        localAudioTrack?.let { peerConnection?.addTrack(it, listOf("O_STREAM")) }
    }
